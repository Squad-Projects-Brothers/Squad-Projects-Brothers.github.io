function aaa() {
    ...
    socket.on('itemsArray', itemsArray => {
        ...
        itemsArray.forEach(item => {
            ...
            itemDiv.append(`<div class="titulo-mesa text-primary-emphasis bg-primary-subtle border border-primary-subtle rounded-3">Mesa: ${mesa}</div>`);

            // Cria um elemento de lista não ordenada para os produtos do item
            const produtosList = $('<table class=" table "></table>');
            const tabelaList = $('<tr><th class="col text-center">Produto</th><th class="col text-center">Quantidade</th><th class="col text-center">Status</th></tr>');
            produtosList.append(tabelaList);
            // Percorre a lista de produtos do item e cria elementos de lista para cada produto
            produtos.forEach(produto => {
                const produtoElement = $(`<tr><td class="col"> ${produto.nome}</td><td class="col"> ${produto.quantidade}</td><td class="col"> ${produto.status}</td></tr>`);
